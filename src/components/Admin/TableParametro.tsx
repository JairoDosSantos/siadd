...
type ParametroType = {
    ...
    descricao: string;
    peso: number;
    dimensao: DimensaoType
}

const TableParametro: React.FC = () => {

    let [searchParametro, setSearchParametro] = useState("");
    ...
    const { data, isLoading, error } = useQuery('fetchParametros', getAllParametro)


    const filteredParameters = searchParametro ? data?.filter((parametro) => parametro.descricao.toLowerCase().includes(searchParametro.toLocaleLowerCase())) : []
    return (
                ...
                rounded-sm border border-gray-200">
                    <header className="px-5 py-4 border-b border-gray-100 w-full">
                        ...
                        bg-gray-50  flex items-center px-4 gap-4  w-full">
                            ...
                            <input
                                onChange={(event) => setSearchParametro(event.target.value)}
                                type="search"
                                id='search'
                            ...
                            <tbody className="text-sm divide-y divide-gray-100">
                                {/**               <!-- record 1 --> */}
                                {
                                    isLoading ? (
                                        ...
                                        !searchParametro ? data?.map((parametro, index) => (
                                            <tr
                                                key={index}
                                                className=''>
                                                <td className="p-2">
                                                    <input type="checkbox" className="w-5 h-5" value={parametro.id}
                                                    />
                                                </td>
                                            ...
                                            (parametroFiltrado) => (
                                                <tr
                                                    key={parametroFiltrado.id}
                                                    className=''>
                                                    <td className="p-2">
                                                        <input type="checkbox" className="w-5 h-5" value={parametroFiltrado.id}
                                                        />
                                                    </td>

                                                    <td className="p-2">
                                                        <span className="font-medium text-gray-800">
                                                            {parametroFiltrado.descricao}
                                                        </span>
                                                    </td>
                                                    <td className="p-2">
                                                        <span className="font-medium text-gray-800">
                                                            {parametroFiltrado.peso}
                                                        </span>
                                                    </td>

                                                    <td className="p-2">
                                                        <span className="font-medium text-gray-800">
                                                            {parametroFiltrado.dimensao.descricao}
                                                        </span>
                                                    </td>
                                                    <td className="p-2 ">
                                                        <span className="flex justify-center ">
