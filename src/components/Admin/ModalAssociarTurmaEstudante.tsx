...
export default function ModalAssociarTurmaEstudante({ isOpen, setIsOpen, idEstudante }: ModalProps) {
    ...
    const fetchAnoAcademicoDocente = async () => {
        try {
            ...
            const anoAcademico = await api.get("/anoAcademico");
    ...
    const { data: semestre } = useQuery("fetchSemestreEstudante", fetchSemestreDocente)
    const { data: anoAcademico } = useQuery("fetchAnoAcademicoDocente", fetchAnoAcademicoDocente)

    const onSubmit: SubmitHandler<formType> = async ({ anoAcademico_id, anoLectivo_id, curso_id, semestre_id, turma_id }) => {

        try {

            setLoad(true)

            const submitAPI = await api.post("/turmaEstudante", {
                ano_academico_id: anoAcademico_id,
                            ...
                            >
                                <Dialog.Panel className="w-full max-w-2xl h-auto transform overflow-hidden rounded-2xl
                                 bg-white p-6 text-left align-middle shadow-xl transition-all">
                                    ...
                                    <div className="mt-2">
                                        <p className={`text-xs text-center text-red-500 ${errors.idEstudante && 'animate-pulse'}`}>
                                        ...
                                        <form
                                            ...
                                            <div className='flex lg:flex-col flex-row gap-4 w-full '>
                                                ...
                                                <div className='flex flex-col w-full gap-2'>
                                                    ...
                                                    >
                                                        {
                                                            anoLectivo?.map((anoLectivo, index) => (
                                                                <option
                                                                    key={index}
                                                                    value={anoLectivo.id}>{anoLectivo.descricao}</option>
                                                            ))
