...
export default function CadastrarDisciplina({ isOpen, setIsOpen }: CadastrarDisciplinaProps) {

    const { register, handleSubmit, formState: { errors }, reset } = useForm<disciplinaType>();
    ...
    const onSubmit: SubmitHandler<disciplinaType> = async (data) => {

        try {

            setLoad(true)
            const submitAPI = await api.post("/disciplina", {
                descricao: data.disciplina
            })
                            ...
                            >
                                <Dialog.Panel className="w-full max-w-2xl h-auto transform overflow-hidden rounded-2xl
                                 bg-white p-6 text-left align-middle shadow-xl transition-all">
                                    ...
                                    >
                                        <FiBookOpen />  <span>Cadastrar disciplina</span>
                                    </Dialog.Title>
                                    <div className="mt-2">
                                        <p className={`text-xs text-center text-red-500 ${errors.disciplina && 'animate-pulse'}`}>
                                        ...
                                        <form
                                            ...
                                            <div className='flex flex-col lg:flex-row gap-4 w-full '>
                                                <div className='flex flex-col w-full gap-2'>
                                                    <label
                                                        className='text-lg'
                                                        htmlFor="nomeProf">Disciplina*: </label>
                                                    <input
                                                        id='disciplina'
                                                        {...register("disciplina", { required: true, minLength: 2 })}
                                                        className={`w-full rounded-lg  shadow ${errors.disciplina && 'border-2 border-red-700'} border-gray-300`}
