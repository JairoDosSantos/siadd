...
export default function ModalEditDimensao({ isOpen, setIsOpen, dimensao }: CadastrarDimensaoProps) {

    const { register, handleSubmit, watch, formState: { errors, isValid }, reset } = useForm<dimensaoType>();
    ...
    const onSubmit: SubmitHandler<dimensaoType> = async (data) => {
        try {
            setLoad(true)
            const submitAPI = await api.put(`/dimensao/${dimensao.id}`, {
                                        ...
                                        <form
                                            ...
                                            <div className='flex flex-col lg:flex-row gap-4 w-full '>
                                                <div className='flex flex-col w-full gap-2'>
                                                    ...
                                                    <input
                                                        defaultValue={dimensao.descricao}
                                                        id='dimensão'
                                                    ...
                                                    />
                                                    <input
                                                        defaultValue={dimensao.peso}
                                                        id='peso'
                                                        {...register("peso", { required: true, minLength: 2 })}
                                                        className={`w-full rounded-lg  shadow ${errors.descricao && 'border-2 border-red-700'} border-gray-300`}
                                                        placeholder='Dimensão*'
                                                        type="text"
                                                    />
