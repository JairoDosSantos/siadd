...
export default function ModalCadastrarParametro({ isOpen, setIsOpen }: CadastrarIndicadorProps) {

    const { register, handleSubmit, formState: { errors }, reset } = useForm<ParametroType>();
    ...
    const getAllParametro = async () => {
        ...
        try {
            ...
            const getParametro = await api.get('/dimensao')
            return getParametro.data as DimensaoType[]
    ...
    const { data } = useQuery('fetchParametrosDimensao1', getAllParametro)



    const onSubmit: SubmitHandler<ParametroType> = async ({ descricao, dimensao_id, peso }) => {

        try {

            setLoad(true)

            const submitAPI = await api.post("/parametro", {
                descricao,
                peso,
                dimensao_id
            })
                            ...
                            >
                                <Dialog.Panel className="w-full max-w-2xl h-auto transform overflow-hidden rounded-2xl
                                 bg-white p-6 text-left align-middle shadow-xl transition-all">
                                    ...
                                    >
                                        <VscSettings />  <span>Cadastrar Parâmetro</span>
                                    </Dialog.Title>
                                    <div className="mt-2">
                                        ...
                                        <form
                                            ...
                                            <div className='flex flex-col lg:flex-row gap-4 w-full '>
                                                <div className='flex flex-col w-full gap-2'>
                                                    <label
                                                        className='text-lg'
                                                        htmlFor="parametro">Parâmetro *: </label>
                                                    <input
                                                        id='parametro'
                                                        {...register("descricao")}
                                                    ...
                                                    <label
                                                        className='text-lg'
                                                        htmlFor="indicador">Dimensão *: </label>
                                                    <select
                                                        placeholder='Parâmetro'
                                                        className={`w-full rounded-lg  shadow ${errors.dimensao_id && 'border-2 border-red-700'} border-gray-300`}
                                                        {...register('dimensao_id')}>
                                                        {
                                                            data?.map((dimensao, index) => (
                                                                <option
                                                                    key={index}
                                                                    value={dimensao.id}>{dimensao.descricao}</option>
                                                            ))
                                                        }
                                                    </select>
                                                    <label
                                                        className='text-lg'
                                                        htmlFor="indicador">Peso *: </label>
                                                    <input
                                                        id='peso'
                                                        {...register("peso")}
