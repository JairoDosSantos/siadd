...
import { generalType } from '../../pages/admin/eleger_membros_cad';
import { api } from '../../service/api';
const SweetAlert2 = dynamic(() => import('react-sweetalert2'), { ssr: false })
type CadastrarCADProps = {
    isOpen: boolean;
...
export type cadType = {
    descricao: string
    id?: string;
    periodo_avaliacao: number

};



export default function CadastrarCAD({ isOpen, setIsOpen }: CadastrarCADProps) {

    const { register, handleSubmit, formState: { errors }, reset } = useForm<cadType>();
    const [showSuccessAlert, setShowSuccessAlert] = useState(false)
    const [showErrorAlert, setShowErrorAlert] = useState(false)
    const [load, setLoad] = useState(false)

    const getPerioAvaliacao = async () => {
        try {

            const response = await api.get("/periodoAvaliacao")
    ...
    const { data } = useQuery("fetchPeriodoCADAvaliacao", getPerioAvaliacao)

    const onSubmit: SubmitHandler<cadType> = async (data) => {
        try {
            setLoad(true)
            const submitAPI = await api.post("/cad", {
                descricao: data.descricao,
                periodo_avaliacao_id: data.periodo_avaliacao
            })
                            ...
                            >
                                <Dialog.Panel className="w-full max-w-2xl h-auto transform overflow-hidden rounded-2xl
                                 bg-white p-6 text-left align-middle shadow-xl transition-all">
                                    ...
                                    >
                                        <AiOutlineUserAdd />  <span>Cadastrar novo CAD</span>
                                    </Dialog.Title>
                                    <div className="mt-2">
                                        ...
                                        <form
                                            ...
                                            <div className='flex flex-col lg:flex-row gap-4 w-full '>
                                                <div className='flex flex-col w-full gap-2'>
                                                    <label
                                                        className='text-lg'
                                                        htmlFor="nomeProf">Descrição do CAD*: </label>
                                                    <input
                                                        id='descricao'
                                                ...
                                                <div className='flex flex-col w-full gap-2'>
                                                    <label
                                                        className='text-lg'
                                                        htmlFor="nomeProf">Período de Avaliação*: </label>
                                                    <select
                                                        className={`w-full rounded-lg  shadow ${errors.periodo_avaliacao?.ref?.value && 'border-2 border-red-700'} border-gray-300`}
                                                        {...register("periodo_avaliacao")}
                                                        id="">

                                                        {
                                                            data?.map((periodo, index) => (
                                                                <option
                                                                    key={index}
                                                                    value={periodo.id}>{periodo.descricao}</option>
                                                            ))
                                                        }
